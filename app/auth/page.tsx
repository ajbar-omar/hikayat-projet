...
export default function AuthPage() {
  ...
  // 1. Fetch Countries in FRENCH using translation field
  useEffect(() => {
    fetch('https://restcountries.com/v3.1/all?fields=translations,idd,cca2,flags')
      .then(res => res.json())
      .then(data => {
        const formatted = data.map((c: any) => ({
          name: c.translations.fra.common, // اللغة الفرنسية هنا
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900 selection:bg-orange-100 overflow-x-hidden" style={{ direction: 'ltr', textAlign: 'left' }}>
      <style>{`
        ...
        .input-field { background-color: #f4f4f4; border: 2px solid transparent; border-radius: 14px; padding: 14px 18px; width: 100%; outline: none; transition: 0.3s; font-size: 15px; }
        .input-field:focus { border-color: #c5772d; background-color: white; box-shadow: 0 4px 12px rgba(197, 119, 45, 0.08); }
        .input-error { border-color: #ff4d4d !important; background-color: #fff5f5; }
        .label-style { font-weight: 600; font-size: 14px; color: #4b5563; margin-bottom: 8px; display: block; }
      ...
      <nav className="flex justify-between items-center px-6 md:px-20 py-8 max-w-[1440px] mx-auto">
        ...
        <button onClick={() => setIsLogin(false)} className="bg-orange-brand text-white px-8 py-3 rounded-2xl font-bold text-sm shadow-lg hover:scale-105 transition">Create account</button>
      </nav>
      ...
      <main className="max-w-[1300px] mx-auto flex flex-col lg:flex-row items-center justify-center px-6 py-10 lg:py-12 gap-16 lg:gap-28">
        ...
        <div className="bg-orange-brand w-full lg:w-[480px] rounded-[55px] p-12 md:p-14 text-white relative overflow-hidden min-h-[620px] flex flex-col shadow-2xl transition-all duration-500">
          <div className="relative z-10">
            <h1 className="text-4xl md:text-5xl font-bold leading-[1.1] mb-8">
              {isLogin ? "Espace Parent Sécurisé" : <>Bienvenue sur <br /> HIKAYAT</>}
            </h1>
            <p className="text-lg opacity-90 mb-12 leading-relaxed max-w-[320px] font-medium">
          ...
          <img src="/kid.png" className="absolute bottom-[-10px] right-[-30px] w-[320px] md:w-[380px] object-contain select-none transition-all duration-500" />
        </div>
        ...
        <div className="w-full max-w-[540px]">
          {status === 'check_email' ? (
             <div className="text-center py-20 animate-in fade-in zoom-in">
                ...
                <h2 className="text-3xl font-bold mb-4 text-gray-900">Vérifiez votre email</h2>
                <button onClick={() => setIsLogin(true)} className="btn-main">Se connecter</button>
             </div>
          ) : (
            <>
              <h2 className="text-4xl font-bold mb-3 text-gray-900 tracking-tight">{isLogin ? "Connexion" : "Créer un compte"}</h2>
              <p className="text-gray-400 mb-12 font-medium">{isLogin ? "Accédez à votre espace parent" : "Remplissez le formulaire pour commencer"}</p>
              ...
              <form onSubmit={handleAuth} className="space-y-6">
                {!isLogin && (
                  <>
                    ...
                    <div className="relative" ref={countryRef}>
                      ...
                      <div className="flex items-center bg-[#f4f4f4] rounded-[14px] border-2 border-transparent focus-within:border-[#c5772d] focus-within:bg-white transition-all overflow-hidden shadow-sm h-[55px]">
                        
                        <div 
                          className="flex items-center gap-2 px-4 h-full cursor-pointer border-r border-gray-200 hover:bg-gray-100 transition min-w-[110px]"
                          onClick={() => setShowCountries(!showCountries)}
                        >
                          <img src={`https://flagcdn.com/w40/${formData.countryIso}.png`} alt="flag" className="w-6 h-auto rounded-[2px] shadow-sm" />
                          <span className="font-bold text-[14px] text-[#374151]">{formData.countryCode}</span>
                          <span className="text-[10px] opacity-40">▼</span>
                        </div>

                        <input 
                          className="flex-1 bg-transparent px-4 h-full outline-none font-bold text-[15px] text-[#1f2937]" 
                        ...
                        {showCountries && (
                          <div className="absolute top-[60px] left-0 w-[280px] bg-white border border-gray-100 rounded-2xl shadow-2xl z-[999] overflow-hidden py-2 animate-in fade-in slide-in-from-top-2">
                            <div className="px-3 mb-2">
                              <input 
                                type="text" 
                                className="w-full p-2 text-sm bg-gray-100 rounded-lg outline-none border-none focus:ring-1 focus:ring-orange-400"
                            ...
                            <div className="max-h-[250px] overflow-y-auto custom-scroll">
                              {filteredCountries.map((c) => (
                                <div 
                                  key={c.iso + c.name}
                                  onClick={() => {
                                    setFormData({...formData, countryCode: c.code, countryIso: c.iso});
                                    setShowCountries(false);
                                  }}
                                  className="flex items-center gap-3 px-4 py-3 hover:bg-orange-50 cursor-pointer transition"
                                >
                                  <img src={c.flag} className="w-6 rounded-sm shadow-sm" alt={c.name} />
                ...
                <div className={isLogin ? "space-y-6" : "grid grid-cols-1 md:grid-cols-2 gap-5"}>
                  <div>
                    ...
                    {emailError && <p className="text-[11px] text-red-500 font-bold mt-2 px-2">⚠️ {emailError}</p>}
                  </div>
                  <div>
                    <label className="label-style">Mot de passe</label>
                    <div className="relative">
                      <input type="password" className="input-field" placeholder="••••••••••••••••" required onChange={e => setFormData({...formData, password: e.target.value})} />
                      <span className="absolute right-5 top-4 opacity-20 cursor-pointer">👁</span>
                    </div>
                    {isLogin && <p className="text-right mt-3 text-xs font-bold text-gray-500 cursor-pointer hover:text-orange-brand transition underline decoration-2 underline-offset-4">Mot de passe oublié ?</p>}
                  </div>
                </div>
